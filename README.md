...
- バージョン: `v0.4.0`
- ファイル: `統合Windowsエディタ-0.4.0-x64.exe`
...
Windowsアプリのv0.4.0では、GitHub CLIとGit認証を初回に設定した後、上部の「GitHubへ同期」ボタンだけで、互換フォルダー内の変更をコミットしてGitHubへ送信できます。アプリの自動保存はローカル保存として常時働き、GitHubへの同期は執筆の区切りで利用者が実行します。別PCやObsidianの変更を取り込む場合は、GitHub同期パネルの「GitHubから取得」を使用します。

初回接続では、互換フォルダーをGit管理し、GitHubリポジトリのoriginを設定したうえで、アプリのGitHub同期パネルから「初回接続を開始」と「接続確認」を実行します。認証情報は作品フォルダーや`project.json`には保存されません。
